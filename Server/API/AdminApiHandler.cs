...
namespace Komodo.Server
{
    public partial class KomodoServer
    {
        static async Task AdminApiHandler(HttpContext ctx)
        {
            ...
            _Logging.Warn(header + "AdminApiHandler unknown endpoint " + ctx.Request.Method.ToString() + " " + ctx.Request.RawUrlWithoutQuery);
            ctx.Response.StatusCode = 400;
            ctx.Response.ContentType = "application/json";
            await ctx.Response.Send(new ErrorResponse(400, "Unknown endpoint.", null).ToJson(true));
            return;
        }
    }
}