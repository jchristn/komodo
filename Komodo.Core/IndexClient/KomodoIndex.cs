...
namespace Komodo.IndexClient
{
    ...
    public class KomodoIndex : IDisposable
    {
        ...
        /// <summary>
        /// Number of entries to attempt to insert into the database at once.
        /// </summary>
        public int BatchInsertSize
        {
            get
            {
                return _BatchInsertSize;
            }
            set
            {
                if (value < 1) throw new ArgumentException("BatchInsertSize must be greater than zero.");
                _BatchInsertSize = value;
            }
        }
        ...
        private int _BatchInsertSize = 512;
        ...
        public ParseResult GetParseResult(string sourceGuid)
        {
            if (String.IsNullOrEmpty(sourceGuid)) throw new ArgumentNullException(nameof(sourceGuid)); 
        ...
        public async Task<IndexResult> Add(SourceDocument sourceDoc, byte[] data, bool parse, PostingsOptions options, string postbackUrl)
        {
            ...
            ret.SourceDocument = _ORM.Insert<SourceDocument>(sourceDoc); 
            ...
            if (parse)
            {
                ...
                if (sourceDoc.Type == DocType.Csv)
                {
                    CsvParser csvParser = new CsvParser();
                    parseResult = csvParser.ParseBytes(data);
                }
                ...
                if (ret.ParseResult != null)
                {
                    ...
                    ret.PostingsResult = postings.Process(ret.ParseResult);
                ...
                if (ret.ParseResult != null)
                {
                    ...
                    ret.ParsedDocument = new ParsedDocument(
                        sourceDoc.GUID,
                        sourceDoc.OwnerGUID,
                        _GUID,
                        sourceDoc.Type,
                        data.Length,
                        ret.PostingsResult.Terms.Count,
                        ret.PostingsResult.Postings.Count);

                    ret.ParsedDocument = _ORM.Insert<ParsedDocument>(ret.ParsedDocument);
                    await _ParsedDocsStorage.Write(ret.ParsedDocument.GUID, "application/json", Common.SerializeJson(ret.ParseResult, true));
                ...
                #region Persist-Postings-Doc

                if (ret.PostingsResult != null)
                {
                    ret.Time.PersistPostingsDocument.Start = DateTime.Now.ToUniversalTime();
                    ret.PostingsDocument = new PostingsDocument(
                        sourceDoc.GUID,
                        sourceDoc.OwnerGUID,
                        _GUID,
                        sourceDoc.Type,
                        ...
                        ret.PostingsResult.Terms.Count,
                        ret.PostingsResult.Postings.Count);
                    ret.PostingsDocument = _ORM.Insert<PostingsDocument>(ret.PostingsDocument);
                    await _PostingsStorage.Write(ret.ParsedDocument.GUID, "application/json", Common.SerializeJson(ret.PostingsResult, true));
                ...
                if (ret.PostingsResult != null && ret.PostingsResult.Normalized != null && ret.PostingsResult.Normalized.Tokens != null)
                {
                    ...
                    // get list of terms
                    List<string> allTerms = ret.PostingsResult.Normalized.Tokens.Select(t => t.Value).ToList();

                    // get term GUIDs that exist
                    DbExpression termGuidSearch = new DbExpression(_ORM.GetColumnName<TermGuid>(nameof(TermGuid.Term)), DbOperators.In, allTerms);
                    List<TermGuid> existingTermGuidObjects = _ORM.SelectMany<TermGuid>(termGuidSearch);
                    List<string> existingTermGuids = new List<string>();
                    if (existingTermGuidObjects != null && existingTermGuidObjects.Count > 0)
                    {
                        existingTermGuids = existingTermGuidObjects.Select(t => t.Term).ToList();
                    }

                    // reduce to new terms
                    List<string> newTerms = allTerms.Except(existingTermGuids).ToList();

                    // create term GUIDs for remainder
                    List<TermGuid> newTermGuidObjects = new List<TermGuid>();
                    foreach (string term in newTerms)
                    {
                        TermGuid tg = new TermGuid();
                        tg.GUID = Guid.NewGuid().ToString();
                        tg.IndexGUID = _GUID;
                        tg.Term = term;
                        newTermGuidObjects.Add(tg);

                        if (newTermGuidObjects.Count >= _BatchInsertSize)
                        {
                            _ORM.InsertMultiple<TermGuid>(newTermGuidObjects);
                            newTermGuidObjects = new List<TermGuid>();
                        }
                    }

                    if (newTermGuidObjects.Count > 0)
                    {
                        _ORM.InsertMultiple<TermGuid>(newTermGuidObjects);
                    }

                    List<TermGuid> allTermGuids = new List<TermGuid>(existingTermGuidObjects);
                    if (newTermGuidObjects.Count > 0) allTermGuids.AddRange(newTermGuidObjects);

                    // create term doc entries
                    List<TermDoc> termDocs = new List<TermDoc>();
                    foreach (TermGuid termGuid in allTermGuids)
                    {
                        TermDoc td = new TermDoc(_GUID, termGuid.GUID, sourceDoc.GUID, ret.ParsedDocument.GUID, ret.PostingsDocument.GUID);
                        termDocs.Add(td);
                        if (termDocs.Count >= _BatchInsertSize)
                        {
                            _ORM.InsertMultiple<TermDoc>(termDocs);
                            termDocs = new List<TermDoc>();
                        }
                    }

                    if (termDocs != null && termDocs.Count > 0)
                    {
                        _ORM.InsertMultiple<TermDoc>(termDocs);
                    }
